...
public class Dmm4jImpl implements Dmm4j {
    ...
    public ItemListResponse getItemList(ItemListParameters parameters) {
        Stream<Map.Entry<String, String>> params = Stream.of(
                ...
                Map.entry("offset", Optional.ofNullable(parameters.getOffset())
                        .map(Objects::toString)),
                Map.entry("sort", Optional.ofNullable(parameters.getSort())
                ...
                Map.entry("article", Optional.ofNullable(parameters.getArticle())
                ...
                Map.entry("lte_date",
                        Optional.ofNullable(parameters.getLteDate())
                                .map(LOCAL_DATE_TIME_FORMATTER::format)),
                Map.entry("mono_stock", Optional.ofNullable(parameters.getMonoStock())
    ...
    public ActressSearchResponse getActressSearch(ActressSearchParameters parameters) {
        Stream<Map.Entry<String, String>> params = Stream.of(
                ...
                Map.entry("keyword", Optional.ofNullable(parameters.getKeyword())),
                Map.entry("gte_bust", Optional.ofNullable(parameters.getGteBust())
                        .map(Objects::toString)),
                Map.entry("lte_bust", Optional.ofNullable(parameters.getLteBust())
                        .map(Objects::toString)),
                Map.entry("gte_waist", Optional.ofNullable(parameters.getGteWaist())
                        .map(Objects::toString)),
                Map.entry("lte_waist", Optional.ofNullable(parameters.getLteWaist())
                        .map(Objects::toString)),
                Map.entry("gte_hip", Optional.ofNullable(parameters.getGteHip())
                        .map(Objects::toString)),
                Map.entry("lte_hip", Optional.ofNullable(parameters.getLteHip())
                        .map(Objects::toString)),
                Map.entry("gte_height", Optional.ofNullable(parameters.getGteHeight())
                        .map(Objects::toString)),
                Map.entry("lte_height", Optional.ofNullable(parameters.getLteHeight())
                        .map(Objects::toString)),
                Map.entry("gte_birthday", Optional.ofNullable(parameters.getGteBirthday())
                        .map(LOCAL_DATE_FORMATTER::format)),
                Map.entry("lte_birthday", Optional.ofNullable(parameters.getLteBirthday())
                        .map(LOCAL_DATE_FORMATTER::format)),
                Map.entry("hits", Optional.ofNullable(parameters.getHits())
                        .map(Objects::toString)),
                Map.entry("offset", Optional.ofNullable(parameters.getOffset())
